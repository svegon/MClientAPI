...
public class CharSummaryStatistics implements CharConsumer {
     ...
     * @since 10
     */
    public CharSummaryStatistics(long count, char min, char max, long sum) throws IllegalArgumentException {
        if (count < 0L) {
    ...
    public void accept(char value) {
        ...
        min = (char) Math.min(min, value);
        max = (char) Math.max(max, value);
    }
    ...
    public void combine(CharSummaryStatistics other) {
        ...
        min = (char) Math.min(min, other.min);
        max = (char) Math.max(max, other.max);
    }
     ...
     * numerical sum compared to a simple summation of {@code doubles}
     * values.
     *
