 ...
 * @since 1.8
 */
public abstract class AbstractPipeline<E_IN, E_OUT, S extends BaseStream<E_OUT, S>> extends PipelineHelper<E_OUT>
    ...
    /**
     * The number of intermediate operations between this pipeline objects
     ...
     * up to and including the operation represented by this pipeline objects.
     * Valid at the point of pipeline preparation for evaluation.
     */
    ...
    @Override
    @SuppressWarnings("unchecked")
    public final <P_IN> boolean copyIntoWithCancel(Sink<P_IN> wrappedSink, Spliterator<P_IN> spliterator) {
        @SuppressWarnings({"rawtypes"})
        AbstractPipeline p = AbstractPipeline.this;
    ...
    @Override
    @SuppressWarnings("unchecked")
    public final <P_IN> Spliterator<E_OUT> wrapSpliterator(Spliterator<P_IN> sourceSpliterator) {
        if (depth == 0) {
     ...
     */
    public <P_IN> Node<E_OUT> opEvaluateParallel(PipelineHelper<E_OUT> helper, Spliterator<P_IN> spliterator,
                                                 IntFunction<E_OUT[]> generator) {
