...
public class FixedBackendCombinedList<E> extends AbstractList<E> implements CombinedList<E> {
    private final ImmutableList<List<E>> cachedLists;

    public FixedBackendCombinedList(ImmutableList<List<E>> cache) {
        cachedLists = cache;
    }

    @SafeVarargs
    public FixedBackendCombinedList(List<? extends E>... collections) {
        this(ImmutableList.copyOf((List<E>[]) collections));
    }

    public FixedBackendCombinedList(Collection<? extends Collection<? extends E>> collections) {
        this(ImmutableList.copyOf((Collection<? extends List<E>>) collections));
    }

    public @NotNull ImmutableList<List<E>> getBackingLists() {
        return cachedLists;
    }
