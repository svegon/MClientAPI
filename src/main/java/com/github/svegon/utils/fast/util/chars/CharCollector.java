...
import com.github.svegon.utils.fast.util.objects.immutable.ImmutableEnumSet;
...
public interface CharCollector<A, R> {
     ...
     */
    ObjectCharConsumer<A> accumulator();
    ...
    static <A, R> CharCollector<A, R> of(Supplier<A> supplier, ObjectCharConsumer<A> accumulator,
                                         BinaryOperator<A> combiner, Function<A, R> finisher,
                                         ImmutableEnumSet<Collector.Characteristics> characteristics) {
        return new CharCollectorImpl<>(Preconditions.checkNotNull(supplier), Preconditions.checkNotNull(accumulator),
    ...
    final class CharCollectorImpl<A, R> implements CharCollector<A, R> {
        private final Supplier<A> supplier;
        private final ObjectCharConsumer<A> accumulator;
        private final BinaryOperator<A> combiner;
        private final Function<A, R> finisher;
        private final ImmutableEnumSet<Collector.Characteristics> characteristics;

        private CharCollectorImpl(Supplier<A> supplier, ObjectCharConsumer<A> accumulator,
                                     BinaryOperator<A> combiner, Function<A, R> finisher,
                                     ImmutableEnumSet<Collector.Characteristics> characteristics) {
        ...
        @Override
        public ObjectCharConsumer<A> accumulator() {
            return accumulator;
        }
