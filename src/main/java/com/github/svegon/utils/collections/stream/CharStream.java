...
import com.github.svegon.utils.fast.util.chars.CharCollector;
import com.github.svegon.utils.fast.util.chars.CharSummaryStatistics;
...
public interface CharStream extends BaseStream<Character, CharStream> {
     ...
     * @return the new stream
     */
    <U> Stream<U> mapToObj(Char2ObjectFunction<? extends U> mapper);
     ...
     * @return the new stream
     */
    BooleanStream mapToBoolean(CharPredicate mapper);
     ...
     * @return the new stream
     */
    ByteStream mapToByte(Char2ByteFunction mapper);
     ...
     * @return the new stream
     */
    ShortStream mapToShort(Char2ShortFunction mapper);
     ...
     * @return the new stream
     */
    IntStream mapToInt(Char2IntFunction mapper);
     ...
     * @return the new stream
     */
    LongStream mapToLong(Char2LongFunction mapper);
     ...
     * @return the new stream
     */
    FloatStream mapToFloat(Char2FloatFunction mapper);
     ...
     * @return the new stream
     */
    DoubleStream mapToDouble(Char2DoubleFunction mapper);
     ...
     */
    CharStream flatMap(Char2ObjectFunction<? extends CharStream> mapper);
     ...
     * stream source (such as {@link #generate(CharSupplier)}) or removing the
     ...
     * {@link #generate(CharSupplier)}) or removing the ordering constraint
     ...
     * @since 9
     */
    default CharStream takeWhile(CharPredicate predicate) {
        Objects.requireNonNull(predicate);
        // Reuses the unordered spliterator, which, when encounter is present,
        // is safe to use as longs as it configured not to split
        return StreamUtil.charStream(new WhileOps.UnorderedWhileSpliterator.OfChar.Taking(spliterator(), true,
     ...
     * @since 9
     */
    default CharStream dropWhile(CharPredicate predicate) {
        Objects.requireNonNull(predicate);
        // Reuses the unordered spliterator, which, when encounter is present,
        // is safe to use as longs as it configured not to split
        return StreamUtil.charStream(new WhileOps.UnorderedWhileSpliterator.OfChar.Dropping(spliterator(),
     ...
     * <pre>{@code
     *     Char sum = numbers.reduce(0, (a, b) -> a+b);
     * }</pre>
     *
     * or more compactly:
     *
     * <pre>{@code
     *     Char sum = numbers.reduce(0, Char::sum);
     * }</pre>
     *
     ...
     * function, and returns an {@code OptionalChar} describing the reduced
     ...
     * }</pre>
     *
     * <p>Like {@link #reduce(char, CharBinaryOperator)}, {@code collect}
     ...
     */
    <R> R collect(Supplier<R> supplier, ObjectCharConsumer<R> accumulator, BiConsumer<R, R> combiner);
     ...
     * {@link #collect(Supplier, ObjectCharConsumer, BiConsumer)}, allowing for reuse of
     ...
     * <pre>{@code
     *     CharList asList = charStream.collect(CollectingUtil.toCharList());
     * }</pre>
     *
     ...
     * @return the result of the reduction
     * @see #collect(Supplier, ObjectCharConsumer, BiConsumer)
     * @see Collectors
     */
    <R, A> R collect(CharCollector<A, R> collector);
     ...
     * Charing-point summation were exact, this method would be
     * equivalent to:
     *
     * <pre>{@code
     *     return reduce(0, Char::sum);
     * }</pre>
     *
     * However, since Charing-point summation is not exact, the above
     * code is not necessarily equivalent to the summation computation
     * done by this method.
     *
     * <p>The value of a Charing-point sum is a function both
     ...
     * numerical sum compared to a simple summation of {@code Char}
     * values.
     *
     ...
     * stream, or an empty OptionalChar if this stream is empty.  The minimum
     * element will be {@code Char.NaN} if any stream element was NaN. Unlike
     * the numerical comparison operators, this method considers negative zero
     * to be strictly smaller than positive zero. This is a special case of a
     * <a href="package-summary.html#Reduction">reduction</a> and is
     * equivalent to:
     * <pre>{@code
     *     return reduce(Char::min);
     * }</pre>
     *
     * <p>This is a <a href="package-summary.html#StreamOps">terminal
     * operation</a>.
     ...
     * stream, or an empty OptionalChar if this stream is empty.  The maximum
     * element will be {@code Char.NaN} if any stream element was NaN. Unlike
     * the numerical comparison operators, this method considers negative zero
     ...
     * <pre>{@code
     *     CharStream s = CharStream.of(1, 2, 3, 4);
    ...
    /**
     * Returns an {@code OptionalChar} describing the arithmetic
     ...
     * to yield more accurate results.
     *
     * @return an {@code OptionalChar} containing the average element of this
     * stream, or an empty optional if the stream is empty
     */
    OptionalDouble average();

    /**
     * Returns a {@code CharSummaryStatistics} describing various summary data
     ...
     * operation</a>.
     *
     * @return a {@code CharSummaryStatistics} describing various summary data
     ...
     * stream, or an empty {@code OptionalChar} if the stream is empty.  If
     * the stream has no encounter order, then any element may be returned.
     *
     * <p>This is a <a href="package-summary.html#StreamOps">short-circuiting
     * terminal operation</a>.
     *
     * @return an {@code OptionalChar} describing the first element of this
     * stream, or an empty {@code OptionalChar} if the stream is empty
     */
    OptionalChar findFirst();

    /**
     * Returns an {@link OptionalChar} describing some element of the stream,
     ...
     *
     * @return an {@code OptionalChar} describing some element of this stream,
     * or an empty {@code OptionalChar} if the stream is empty
     * @see #findFirst()
     */
     ...
     * each boxed to a {@code Char}
     */
    default Stream<Character> boxed() {
        return mapToObj(c -> c);
    }
    ...
    /**
     * Returns a builder for a {@code CharStream}.
     *
    ...
    static CharStream empty() {
        return StreamUtil.charStream(SpliteratorUtil.emptyCharSpliterator(), false);
    }

    /**
     * Returns a sequential {@code CharStream} containing a single element.
     *
     * @param t the single element
     * @return a singleton sequential stream
     */
    static CharStream of(char t) {
        return StreamUtil.charStream(CharSpliterators.singleton(t), false);
    }
     ...
     * @return the new stream
     */
    static CharStream of(char... values) {
        return ArrayUtil.stream(values);
    }

    /**
     * Returns an infinite sequential ordered {@code CharStream} produced by iterative
     * application of a function {@code f} to an initial element {@code seed},
     * producing a {@code Stream} consisting of {@code seed}, {@code f(seed)},
     * {@code f(f(seed))}, etc.
     *
     * <p>The first element (position {@code 0}) in the {@code CharStream}
     ...
     *          a new element
     * @return a new sequential {@code CharStream}
     */
    static CharStream iterate(final char seed, final CharUnaryOperator f) {
        Objects.requireNonNull(f);

        return StreamUtil.charStream(new SpliteratorUtil.CharSpliteratorImpl(Long.MAX_VALUE,
    ...
    /**
     * Returns a sequential ordered {@code CharStream} produced by iterative
     * application of the given {@code next} function to an initial element,
     * conditioned on satisfying the given {@code hasNext} predicate.  The
     * stream terminates as soon as the {@code hasNext} predicate returns false.
     *
     * <p>{@code CharStream.iterate} should produce the same sequence of elements as
     * produced by the corresponding for-loop:
     * <pre>{@code
     *     for (Char index=seed; hasNext.test(index); index = next.applyAsChar(index)) {
     *         ...
     *     }
     * }</pre>
     *
     ...
     * @since 9
     */
    static CharStream iterate(final char seed, final CharPredicate hasNext, final CharUnaryOperator next) {
        ...
        return StreamUtil.charStream(new SpliteratorUtil.CharSpliteratorImpl(Long.MAX_VALUE,
            ...
            public void forEachRemaining(CharConsumer action) {
                ...
                finished = true;
                char t = started ? next.apply(prev) : seed;
     ...
     * generated by the provided {@code CharSupplier}.  This is suitable for
     * generating constant streams, streams of random elements, etc.
     *
     * @param s the {@code CharSupplier} for generated elements
     * @return a new infinite sequential unordered {@code CharStream}
     */
    static CharStream generate(CharSupplier s) {
        Objects.requireNonNull(s);
        return StreamUtil.charStream(new StreamSpliterators.InfiniteSupplyingSpliterator.OfChar(Long.MAX_VALUE, s),
                false);
    }
     ...
     * <pre>{@code
     *     CharStream concat = Stream.of(s1, s2, s3, s4).flatMapToChar(s -> s);
     * }</pre>
     *
     ...
     */
    static CharStream concat(CharStream a, CharStream b) {
        Objects.requireNonNull(a);
        Objects.requireNonNull(b);

        CharSpliterator split = new Streams.ConcatSpliterator.OfChar(a.spliterator(), b.spliterator());
        CharStream stream = StreamUtil.charStream(split, a.isParallel() || b.isParallel());
        return stream.onClose(Streams.composedClose(a, b));
    }

    /**
     * A mutable builder for a {@code CharStream}.
     *
     ...
     *
     * @see CharStream#builder()
     * @since 1.8
     */
    final class Builder implements CharConsumer, CharSpliterator {
        ...
        public CharStream build() {
            ...
            if (c >= 0) {
                ...
                return (c < 2) ? StreamUtil.charStream(this, false)
                        : StreamUtil.charStream(buffer.spliterator(), false);
            }
