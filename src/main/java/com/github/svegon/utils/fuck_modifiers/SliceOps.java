...
public final class SliceOps {
     ...
     *
     * @param upstream A BooleanStream
     * @param skip The number of elements to skip.  Must be >= 0.
     * @param limit The maximum size of the resulting stream, or -1 if no limit
     *        is to be imposed
     */
    public static BooleanStream makeBoolean(final AbstractPipeline<?, Boolean, ?> upstream, final long skip,
                                            final long limit) {
        if (skip < 0) {
            throw new IllegalArgumentException("Skip must be non-negative: " + skip);
        }

        return new BooleanPipeline.StatefulOp<Boolean>(upstream, StreamShape.CHAR_VALUE, flags(limit)) {
            public BooleanSpliterator unorderedSkipLimitSpliterator(BooleanSpliterator s, long skip, long limit,
                                                                 long sizeIfKnown) {
                if (skip <= sizeIfKnown) {
                    // Use just the limit if the number of elements
                    // to skip is <= the known pipeline size
                ...
                return new StreamSpliterators.UnorderedSliceSpliterator.OfBoolean(s, skip, limit);
            }

            @Override
            public <P_IN> Spliterator<Boolean> opEvaluateParallelLazy(final PipelineHelper<Boolean> helper,
                                                                        final Spliterator<P_IN> spliterator) {
                long size = helper.exactOutputSizeIfKnown(spliterator);
                if (size > 0 && spliterator.hasCharacteristics(Spliterator.SUBSIZED)) {
                    return new StreamSpliterators.SliceSpliterator.OfBoolean((BooleanSpliterator)
                            helper.wrapSpliterator(spliterator), skip, calcSliceFence(skip, limit));
                } else if (!StreamOpFlag.ORDERED.isKnown(helper.getStreamAndOpFlags())) {
                    return unorderedSkipLimitSpliterator((BooleanSpliterator) helper.wrapSpliterator(spliterator), skip,
                            limit, size);
                }  else {
                    return new SliceTask<>(this, helper, spliterator, Boolean[]::new, skip, limit).invoke()
                            .spliterator();
                }
            }
            ...
            public <P_IN> Node<Boolean> opEvaluateParallel(PipelineHelper<Boolean> helper,
                ...
                if (size > 0 && spliterator.hasCharacteristics(Spliterator.SUBSIZED)) {
                    ...
                    // wrapping spliterator
                    Spliterator<P_IN> s = sliceSpliterator(helper.getSourceShape(), spliterator, skip, limit);
                    return Nodes.collectBoolean(helper, s, true);
                } else if (!StreamOpFlag.ORDERED.isKnown(helper.getStreamAndOpFlags())) {
                    BooleanSpliterator s =  unorderedSkipLimitSpliterator(
                            (BooleanSpliterator) helper.wrapSpliterator(spliterator), skip, limit, size);
                    // Collect using this pipeline, which is empty and therefore
                    // can be used with the pipeline wrapping spliterator
                    // Note that we cannot create a slice spliterator from
                    // the source spliterator if the pipeline is not SIZED
                    return Nodes.collectBoolean(this, s, true);
                }
                else {
     ...
     * operation may be may be skip-only, limit-only, or skip-and-limit.
     *
     * @param upstream A ByteStream
     * @param skip The number of elements to skip.  Must be >= 0.
     * @param limit The maximum size of the resulting stream, or -1 if no limit
     *        is to be imposed
     */
    public static ByteStream makeByte(final AbstractPipeline<?, Byte, ?> upstream, final long skip,
                                      final long limit) {
        if (skip < 0) {
            throw new IllegalArgumentException("Skip must be non-negative: " + skip);
        }

        return new BytePipeline.StatefulOp<Byte>(upstream, StreamShape.CHAR_VALUE, flags(limit)) {
            public ByteSpliterator unorderedSkipLimitSpliterator(ByteSpliterator s, long skip, long limit,
                                                                 long sizeIfKnown) {
                if (skip <= sizeIfKnown) {
                    // Use just the limit if the number of elements
                    // to skip is <= the known pipeline size
                ...
                return new StreamSpliterators.UnorderedSliceSpliterator.OfByte(s, skip, limit);
            }

            @Override
            public <P_IN> Spliterator<Byte> opEvaluateParallelLazy(final PipelineHelper<Byte> helper,
                                                                        final Spliterator<P_IN> spliterator) {
                long size = helper.exactOutputSizeIfKnown(spliterator);
                if (size > 0 && spliterator.hasCharacteristics(Spliterator.SUBSIZED)) {
                    return new StreamSpliterators.SliceSpliterator.OfByte((ByteSpliterator)
                            helper.wrapSpliterator(spliterator), skip, calcSliceFence(skip, limit));
                } else if (!StreamOpFlag.ORDERED.isKnown(helper.getStreamAndOpFlags())) {
                    return unorderedSkipLimitSpliterator((ByteSpliterator) helper.wrapSpliterator(spliterator), skip,
                            limit, size);
                }  else {
                    return new SliceTask<>(this, helper, spliterator, Byte[]::new, skip, limit).invoke()
                            .spliterator();
                }
            }
            ...
            public <P_IN> Node<Byte> opEvaluateParallel(PipelineHelper<Byte> helper,
                ...
                if (size > 0 && spliterator.hasCharacteristics(Spliterator.SUBSIZED)) {
                    ...
                    // wrapping spliterator
                    Spliterator<P_IN> s = sliceSpliterator(helper.getSourceShape(), spliterator, skip, limit);
                    return Nodes.collectByte(helper, s, true);
                } else if (!StreamOpFlag.ORDERED.isKnown(helper.getStreamAndOpFlags())) {
                    ByteSpliterator s =  unorderedSkipLimitSpliterator(
                            (ByteSpliterator) helper.wrapSpliterator(spliterator), skip, limit, size);
                    // Collect using this pipeline, which is empty and therefore
                    // can be used with the pipeline wrapping spliterator
                    // Note that we cannot create a slice spliterator from
                    // the source spliterator if the pipeline is not SIZED
                    return Nodes.collectByte(this, s, true);
                }
                else {
                    return new SliceTask<>(this, helper, spliterator, generator, skip, limit).invoke();
                }
     ...
     * operation may be may be skip-only, limit-only, or skip-and-limit.
     *
     * @param upstream A ShortStream
     * @param skip The number of elements to skip.  Must be >= 0.
     * @param limit The maximum size of the resulting stream, or -1 if no limit
     *        is to be imposed
     */
    public static ShortStream makeShort(final AbstractPipeline<?, Short, ?> upstream, final long skip,
                                        final long limit) {
        if (skip < 0) {
            throw new IllegalArgumentException("Skip must be non-negative: " + skip);
        }

        return new ShortPipeline.StatefulOp<Short>(upstream, StreamShape.CHAR_VALUE, flags(limit)) {
            public ShortSpliterator unorderedSkipLimitSpliterator(ShortSpliterator s, long skip, long limit,
                                                                 long sizeIfKnown) {
                if (skip <= sizeIfKnown) {
                    // Use just the limit if the number of elements
                    // to skip is <= the known pipeline size
                ...
                return new StreamSpliterators.UnorderedSliceSpliterator.OfShort(s, skip, limit);
            }

            @Override
            public <P_IN> Spliterator<Short> opEvaluateParallelLazy(final PipelineHelper<Short> helper,
                                                                        final Spliterator<P_IN> spliterator) {
                long size = helper.exactOutputSizeIfKnown(spliterator);
                if (size > 0 && spliterator.hasCharacteristics(Spliterator.SUBSIZED)) {
                    return new StreamSpliterators.SliceSpliterator.OfShort((ShortSpliterator)
                            helper.wrapSpliterator(spliterator), skip, calcSliceFence(skip, limit));
                } else if (!StreamOpFlag.ORDERED.isKnown(helper.getStreamAndOpFlags())) {
                    return unorderedSkipLimitSpliterator((ShortSpliterator) helper.wrapSpliterator(spliterator), skip,
                            limit, size);
                }  else {
                    return new SliceTask<>(this, helper, spliterator, Short[]::new, skip, limit).invoke()
                            .spliterator();
                }
            }
            ...
            public <P_IN> Node<Short> opEvaluateParallel(PipelineHelper<Short> helper,
                ...
                if (size > 0 && spliterator.hasCharacteristics(Spliterator.SUBSIZED)) {
                    ...
                    // wrapping spliterator
                    Spliterator<P_IN> s = sliceSpliterator(helper.getSourceShape(), spliterator, skip, limit);
                    return Nodes.collectShort(helper, s, true);
                } else if (!StreamOpFlag.ORDERED.isKnown(helper.getStreamAndOpFlags())) {
                    ShortSpliterator s =  unorderedSkipLimitSpliterator(
                            (ShortSpliterator) helper.wrapSpliterator(spliterator), skip, limit, size);
                    // Collect using this pipeline, which is empty and therefore
                    // can be used with the pipeline wrapping spliterator
                    // Note that we cannot create a slice spliterator from
                    // the source spliterator if the pipeline is not SIZED
                    return Nodes.collectShort(this, s, true);
                }
                else {
                    return new SliceTask<>(this, helper, spliterator, generator, skip, limit).invoke();
                }
     ...
     * operation may be may be skip-only, limit-only, or skip-and-limit.
     *
     * @param upstream A CharStream
     * @param skip The number of elements to skip.  Must be >= 0.
     * @param limit The maximum size of the resulting stream, or -1 if no limit
     *        is to be imposed
     */
    public static CharStream makeChar(final AbstractPipeline<?, Character, ?> upstream, final long skip,
                                      final long limit) {
        if (skip < 0) {
            throw new IllegalArgumentException("Skip must be non-negative: " + skip);
        }
        ...
        return new CharPipeline.StatefulOp<Character>(upstream, StreamShape.CHAR_VALUE, flags(limit)) {
            public CharSpliterator unorderedSkipLimitSpliterator(CharSpliterator s, long skip, long limit,
                                                                  long sizeIfKnown) {
                if (skip <= sizeIfKnown) {
                    // Use just the limit if the number of elements
                    // to skip is <= the known pipeline size
                ...
                return new StreamSpliterators.UnorderedSliceSpliterator.OfChar(s, skip, limit);
            }

            @Override
            public <P_IN> Spliterator<Character> opEvaluateParallelLazy(final PipelineHelper<Character> helper,
                                                                        final Spliterator<P_IN> spliterator) {
                long size = helper.exactOutputSizeIfKnown(spliterator);
                if (size > 0 && spliterator.hasCharacteristics(Spliterator.SUBSIZED)) {
                    return new StreamSpliterators.SliceSpliterator.OfChar((CharSpliterator)
                            helper.wrapSpliterator(spliterator), skip, calcSliceFence(skip, limit));
                } else if (!StreamOpFlag.ORDERED.isKnown(helper.getStreamAndOpFlags())) {
                    return unorderedSkipLimitSpliterator((CharSpliterator) helper.wrapSpliterator(spliterator), skip,
                            limit, size);
                }  else {
                    return new SliceTask<>(this, helper, spliterator, Character[]::new, skip, limit).invoke()
                            .spliterator();
                }
            }
            ...
            public <P_IN> Node<Character> opEvaluateParallel(PipelineHelper<Character> helper,
                ...
                if (size > 0 && spliterator.hasCharacteristics(Spliterator.SUBSIZED)) {
                    ...
                    // wrapping spliterator
                    Spliterator<P_IN> s = sliceSpliterator(helper.getSourceShape(), spliterator, skip, limit);
                    return Nodes.collectChar(helper, s, true);
                } else if (!StreamOpFlag.ORDERED.isKnown(helper.getStreamAndOpFlags())) {
                    CharSpliterator s =  unorderedSkipLimitSpliterator(
                            (CharSpliterator) helper.wrapSpliterator(spliterator), skip, limit, size);
                    // Collect using this pipeline, which is empty and therefore
                    // can be used with the pipeline wrapping spliterator
                    // Note that we cannot create a slice spliterator from
                    // the source spliterator if the pipeline is not SIZED
                    return Nodes.collectChar(this, s, true);
                }
                else {
                    return new SliceTask<>(this, helper, spliterator, generator, skip, limit).invoke();
                }
     ...
     *        is to be imposed
     */
    public static FloatStream makeFloat(final AbstractPipeline<?, Float, ?> upstream, final long skip,
                                        final long limit) {
        if (skip < 0) {
            throw new IllegalArgumentException("Skip must be non-negative: " + skip);
        }
        ...
        return new FloatPipeline.StatefulOp<Float>(upstream, StreamShape.FLOAT_VALUE, flags(limit)) {
            public FloatSpliterator unorderedSkipLimitSpliterator(FloatSpliterator s, long skip, long limit,
                                                                  long sizeIfKnown) {
                if (skip <= sizeIfKnown) {
                    // Use just the limit if the number of elements
                    // to skip is <= the known pipeline size
                ...
                return new StreamSpliterators.UnorderedSliceSpliterator.OfFloat(s, skip, limit);
            }

            @Override
            public <P_IN> Spliterator<Float> opEvaluateParallelLazy(final PipelineHelper<Float> helper,
                                                                    final Spliterator<P_IN> spliterator) {
                long size = helper.exactOutputSizeIfKnown(spliterator);
                if (size > 0 && spliterator.hasCharacteristics(Spliterator.SUBSIZED)) {
                    return new StreamSpliterators.SliceSpliterator.OfFloat((FloatSpliterator)
                            helper.wrapSpliterator(spliterator), skip, calcSliceFence(skip, limit));
                } else if (!StreamOpFlag.ORDERED.isKnown(helper.getStreamAndOpFlags())) {
                    return unorderedSkipLimitSpliterator((FloatSpliterator) helper.wrapSpliterator(spliterator), skip,
                            limit, size);
                }  else {
                    return new SliceTask<>(this, helper, spliterator, Float[]::new, skip, limit).invoke()
                            .spliterator();
                }
            }
            ...
            public <P_IN> Node<Float> opEvaluateParallel(PipelineHelper<Float> helper,
                ...
                if (size > 0 && spliterator.hasCharacteristics(Spliterator.SUBSIZED)) {
                    ...
                    // wrapping spliterator
                    Spliterator<P_IN> s = sliceSpliterator(helper.getSourceShape(), spliterator, skip, limit);
                    return Nodes.collectFloat(helper, s, true);
                } else if (!StreamOpFlag.ORDERED.isKnown(helper.getStreamAndOpFlags())) {
                    FloatSpliterator s =  unorderedSkipLimitSpliterator(
                            (FloatSpliterator) helper.wrapSpliterator(spliterator),
                            skip, limit, size);
                    // Collect using this pipeline, which is empty and therefore
                    // can be used with the pipeline wrapping spliterator
                    // Note that we cannot create a slice spliterator from
                    // the source spliterator if the pipeline is not SIZED
                    return Nodes.collectFloat(this, s, true);
                }
                else {
                    return new SliceTask<>(this, helper, spliterator, generator, skip, limit).invoke();
                }
    ...
    private static final class SliceTask<P_IN, P_OUT>
        ...
        public SliceTask(AbstractPipeline<P_OUT, P_OUT, ?> op,
