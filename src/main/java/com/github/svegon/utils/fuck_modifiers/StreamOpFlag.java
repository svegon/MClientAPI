 ...
 * flag bits are combined into an {@code ints} bit set.  <em>Stream flags</em>
 ...
 * <pre> {@code
 *     ints flagsForSourceStage = combineOpFlags(sourceFlags, INITIAL_OPS_VALUE);
 * }</pre>
 *
 ...
 * <pre> {@code
 *     ints flagsForThisStage = combineOpFlags(flagsForPreviousStage, thisOpFlags);
 * }</pre>
 *
