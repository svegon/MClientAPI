...
import io.github.svegon.utils.multithreading.ReentrantThreadExecutor
...
class RenderUtil private constructor() {
    init {
        throw UnsupportedOperationException()
    }

    private class RenderThreadExecutor : ReentrantThreadExecutor<Runnable?>("render-executor") {
        ...
        override fun createTask(runnable: Runnable): Runnable {
            return runnable
        }
    ...
    companion object {
        val RENDER_THREAD_EXECUTOR: ReentrantThreadExecutor<Runnable> = RenderThreadExecutor()

        fun color4FToI(a: FloatArray): Int {
            return (a[0] * 255f).toInt() shl 24 or ((a[1] * 255f).toInt() shl 16) or ((a[2] * 255f).toInt() shl 8) or ((a[3] * 255f).toInt())
        }
        ...
        fun setShaderRed(red: Float) {
            RenderSystem.getShaderColor().get(0) = red
        }

        fun setShaderGreen(green: Float) {
            RenderSystem.getShaderColor().get(1) = green
        }

        fun setShaderBlue(blue: Float) {
            RenderSystem.getShaderColor().get(2) = blue
        }

        fun setShaderAlpha(alpha: Float) {
            RenderSystem.getShaderColor().get(3) = alpha
        }
